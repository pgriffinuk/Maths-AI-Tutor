...
// Sub-topic pilot: a handful of the most overly-broad topics (below) were
// each split into 2-3 narrower ones - a scoped pilot on the clearest cases,
// not a full restructure of every course's topic list. Any existing attempt
// history recorded against the old bundled topic name (e.g. "Quadratic
// equations (factorising, formula, completing the square)") no longer
// exactly matches any topic string in the split-out lists below. That's
// expected and acceptable: the historical data isn't lost or deleted, it
// just won't be picked up by anything that matches on the current topics
// array (Guided Path status, streak/badge topic-grouping, prerequisite
// lookups) - those will just treat the new sub-topics as not-started.
