...
// Confirmed: there is no attribute allow-list here at all, only the tag
// allow-list above plus the two attribute-level strips below (event
// handlers, javascript: hrefs) - so the `id` attribute an AI-generated
// diagram uses to label its distinct parts (see lib/socraticTutor.js and
// app/components/DiagramPanel.js's region highlighting) already passes
// through untouched. Nothing to add here for that to work.
