...
// as it does now (never locked, no rerouting). Where a topic used to be a
// single bundled entry that's since been split into narrower sub-topics
// (see the "Sub-topic pilot" note in lib/levels.js), any prerequisite that
// pointed at the old bundled name now points at whichever new sub-topic is
// the most fundamental of the set, not all of them.
export const TOPIC_PREREQUISITES = {
  ...
  'gcse-higher': {
    'Simultaneous equations (linear and quadratic)': ['Factorising quadratics'],
  ...
  'alevel-pure': {
    ...
    // Was 'Trigonometric ratios, identities and equations' before that topic
    // was split into three (sub-topic pilot, see lib/levels.js) - the
    // "graphs" prerequisite maps most naturally onto the first of the three.
