...
Return ONLY valid JSON, no markdown fences, no preamble, with exactly this field: messages (array of message objects, each { text: string, diagram: string|null } - split your reply into 1-3 of these objects as described above, each one a short, self-contained chunk of the reply, in the order they should appear. For a chunk involving geometry, graphs, trigonometry, vectors, fractions, or data, include a simple SVG diagram as that object's 'diagram' field - raw SVG markup only, viewBox="0 0 300 200", using only these elements: svg, g, path, circle, rect, line, polyline, polygon, text, ellipse. No script tags, no external references, no event handler attributes. Keep diagrams simple and clean - basic shapes, axes, labelled points - not attempting photorealistic or highly detailed drawings. For chunks with no natural visual, leave diagram as null). Since 'text' will contain LaTeX delimiters with backslashes (e.g. $\\frac{a}{b}$), remember to escape every backslash as \\\\ within the JSON string so the output remains valid JSON.`;
