...
// A main topic's aggregate status across its own sub-topics (the { name,
// subtopics } shape from lib/levels.js) - the same three-way rule Guided
// Path already applies inline when grouping a multi-subtopic topic:
// 'mastered' only once every sub-topic is, 'in-progress' if any sub-topic
// has been touched at all, otherwise 'not-started'.
export function computeMainTopicStatus(attempts, course, board, mainTopic) {
  const statuses = mainTopic.subtopics.map((st) => computeTopicStatus(attempts, course, board, st));
  if (statuses.every((s) => s === 'mastered')) return 'mastered';
  if (statuses.some((s) => s !== 'not-started')) return 'in-progress';
  return 'not-started';
}

// A mastered topic left untouched this long is due a spaced-review nudge -
// long enough not to nag right after every short gap, short enough to still
// catch real skill decay before it sets in.
const REVIEW_DUE_DAYS = 14;

// Spaced review: mastered topics the student hasn't practiced in a while,
// most overdue first. Computed straight from attempt history (same
// attempts already loaded for rewards/Guided Path) - no separate table.
export function findDueReviews(attempts) {
  const byTopic = {};
  for (const a of attempts || []) {
    const key = `${a.course || ''}::${a.board || ''}::${a.topic}`;
  ...
  for (const [key, list] of Object.entries(byTopic)) {
    const [course, board, topic] = key.split('::');
    if (computeTopicStatus(attempts, course, board, topic) !== 'mastered') continue;
    const mostRecent = list.reduce((latest, a) => (new Date(a.created_at) > new Date(latest.created_at) ? a : latest));
    const daysSinceLastPractice = Math.floor((now - new Date(mostRecent.created_at).getTime()) / 86400000);
    if (daysSinceLastPractice >= REVIEW_DUE_DAYS) {
      due.push({ board, course, topic, daysSinceLastPractice });
    }
  }
  return due.sort((a, b) => b.daysSinceLastPractice - a.daysSinceLastPractice);
}
