...
// Some routes (currently /api/chat and /api/anon-chat) ask the model to
// write LaTeX with backslashes (\sqrt, \frac, \theta, \pi, ...) inside a
// JSON string value, and to double every backslash so the JSON stays
// valid - a fragile ask in practice. A stray single backslash either
// throws a JSON syntax error outright (\s, \p, \q, ... aren't legal JSON
// escapes) or, worse, silently corrupts the text (\t, \f, \b, \r ARE
// legal escapes, so JSON.parse just eats the backslash and misreads the
// next letter as a control character instead of throwing). This
// defensively re-escapes any backslash that isn't one of the few this app
// actually relies on - \\, \", \n (a real newline inside a bullet-list
// "text" value), or \uXXXX. The first alternative matches an already-
// doubled \\ pair as one atomic unit and leaves it untouched; without
// that, a correctly-escaped pair gets its second backslash re-examined on
// its own and doubled again (turning a clean \\ into a corrupting \\\,
// verified against exactly this case before landing).
function fixUnescapedBackslashes(jsonText) {
...
// userText is normally a plain string, but may instead be an Anthropic
// content-block array (e.g. an image block alongside a text block, for
// routes that accept a photo attachment) - it's passed straight through
// as the user message's `content`, which the Messages API accepts as
// either shape, so every existing string-passing caller is unaffected.
export async function callClaude({ system, userText, expectJson, maxTokens = DEFAULT_MAX_TOKENS }) {
  ...
  for (let parseAttempt = 0; parseAttempt < 2; parseAttempt++) {
    ...
    try {
      return JSON.parse(fixUnescapedBackslashes(cleaned));
    } catch (err) {
      ...
      // Attach the best-effort cleaned text so a caller whose expected
      // shape has a sensible plain-text fallback (see /api/chat and
      // /api/anon-chat) can degrade gracefully instead of failing outright.
      throw Object.assign(
        claudeError('The AI returned a response in an unexpected format.', 'invalid_response'),
        { rawText: cleaned }
      );
    }
  }
