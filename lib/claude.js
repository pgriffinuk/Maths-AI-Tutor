...
async function requestOnce({ system, userText }) {
...
async function fetchClaudeResponse({ system, userText }) {
  for (let attempt = 0; attempt < 2; attempt++) {
    let response;
    try {
      response = await requestOnce({ system, userText });
    } catch (err) {
...
export async function callClaude({ system, userText, expectJson }) {
  ...
  for (let parseAttempt = 0; parseAttempt < 2; parseAttempt++) {
    const response = await fetchClaudeResponse({ system, userText });
