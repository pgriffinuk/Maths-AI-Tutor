...
export async function POST(req) {
  try {
    const { topic, history, course, accessToken } = await req.json();
    ...
    const userText = `Topic: ${topic}. Write one question of medium difficulty (worth roughly 3-4 marks) appropriate to the level described above.${historyNote}`;
